...
# config, registers the plugin in opencode.json, and prompts the user
# to star the GitHub repo.
#
# Usage:
...
echo ""
echo "  ╔══════════════════════════════════════════════════╗"
echo "  ║          elicify-vertex installed!               ║"
echo "  ║   Make any model work like a senior engineer      ║"
echo "  ╚══════════════════════════════════════════════════╝"
echo ""

# --- install skill --------------------------------------------------------
if [[ -f "$SOURCE_SKILL" ]]; then
  echo "  Installing /vertex skill..."
...
if [[ -f "$SOURCE_AGENT" ]]; then
  echo ""
  echo "  Installing Elicify-Vertex-Agent..."
...
echo ""
echo "  ─────────────────────────────────────────────────"
echo "  Enjoying elicify-vertex? It's free and open source."
echo "  A GitHub star helps other developers discover it."
echo "  ─────────────────────────────────────────────────"
echo ""

if [[ "${VERTEX_NO_STAR:-0}" == "1" ]]; then
  echo "  (star prompt skipped — VERTEX_NO_STAR=1)"
elif [[ ! -t 0 ]]; then
  ...
  if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    if gh api --method PUT /user/starred/elicify-ai/elicify-vertex 2>/dev/null; then
      echo "  ⭐ Starred! Thank you for supporting elicify-vertex."
    else
      echo "  Star us: https://github.com/elicify-ai/elicify-vertex"
    fi
  else
    echo "  Star us: https://github.com/elicify-ai/elicify-vertex"
  fi
  ...
  printf "  Star elicify-ai/elicify-vertex on GitHub? [Y/n] "
  ...
  case "$(echo "$response" | tr '[:upper:]' '[:lower:]')" in
    y|yes)
      if gh api --method PUT /user/starred/elicify-ai/elicify-vertex 2>/dev/null; then
        ...
        echo "  Could not star automatically. Star manually:"
        echo "  https://github.com/elicify-ai/elicify-vertex"
      fi
      ;;
    *)
      echo "  No problem! You can star later: https://github.com/elicify-ai/elicify-vertex"
      ;;
  esac
else
...
echo "  Agent: Elicify-Vertex-Agent  |  Skill: /vertex"
echo ""
