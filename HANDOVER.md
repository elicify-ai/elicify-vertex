...
- Two full `/goal` cycles this session closed **every** item in `docs/CODE-ISSUES-FROM-PROMPT-AUDIT.md` (C-1 through C-16), including:
  - **Verifier redesign** (`docs/VERIFIER-PROMPT.md`): richer verdict shape (`fit/summary/gaps`), richer 5-field payload (adds real `lastResponse`/`recentTranscript` fetched via `client.session.messages`, not a synthetic summary), scanned for secrets *before* truncation.
...
- **The completion verifier never fired.** It's gated behind *every* story reaching `"complete"` via the deterministic gate first (`gate.ts`'s `appendVerifierCloseOut` returns early otherwise) — so the verifier can never rescue a session that stalls on the deterministic gate, which is exactly the failure mode it exists for.
...
2. **The verifier becomes the sole arbiter of story/plan completion**, triggered at `session.idle` (same trigger point as today, not a new one).
3. **Verifier gets real tools** — read / grep / glob / bash (to independently re-run verifiers itself, exactly as done by hand this session) — **no write/edit**. This reverses an earlier deliberate zero-tool security decision (`src/v2/subturn.ts`'s permission wildcard-deny); the user explicitly chose this.
4. **Verifier output is structured, per-acceptance-item feedback** — which criteria are met, which aren't, what's specifically missing — not prose fit/summary.
...
9. **Adopt from `opencode-goal-plugin`** (cloned to `/tmp/claude-1000/.../scratchpad/opencode-goal-plugin/`): task-blocking awareness (defer nudging while a delegated subagent is mid-delegation) and stall detection (`no_progress_token_threshold`/`max_no_progress_turns`). Note: that repo does **not** have a verifier or acceptance-criteria-writing step — its whole completion model is self-report plus nagging; use it only for the continuation/watchdog machinery.
...
- The user was asked whether to run this through `/plan-spec` first given the scope (touches plan data model, verifier, idle gate, composer/findings, agent prompt) — **that question was not yet answered** when the handover was requested. It's the natural next decision.
