...
> its tasks are done (then the judge audits it). Promotion is
...
Supersedes the completion model described in `docs/JUDGE-PROMPT.md` and the
judge-gating described in `docs/REQUIREMENTS-IDLE-COMPLETION-GATE.md`. The
...
receipts, judge never fired). The nine agreed points are HANDOVER.md's
...
the **completion judge** — a tool-using subturn with read/grep/glob/list/bash
...
2. **Judge is the sole arbiter** — `src/v2/wiring/gate.ts` `handleJudgeAudit`,
   runs at every idle with an unjudged claim; the old `appendJudgeCloseOut`
   (gated behind all-stories-complete) is gone.
3. **Judge has read-only tools** — `src/v2/wiring/config.ts` `JUDGE_PERMISSION`
   (deliberate reversal of the zero-tool decision; `vertex-intake` stays
   zero-tool), `src/v2/subturn.ts` `ProbePolicy`/`JUDGE_PROBE_POLICY`/
   `buildToolPolicyMap`. Judge budget default 300s (`VERTEX_JUDGE_BUDGET_MS`).
4. **Structured per-item verdicts** — `src/v2/judge.ts` `JudgeVerdict
...
5. **Acceptance items are functional claims** — tool descriptions,
   `agents/elicify-vertex-agent.md` `<planning_in_waves>`, and the judge's
   plan-digest payload field (`buildJudgePayload`'s `plan`).
...
8. **Constructive continuations** — `formatJudgeReverts` (gate.ts) and the
   judge-stamp-aware `incompletePlanFinding`/`activeStoryPrescription`
...
- Fail-open everywhere: judge unavailable/malformed → claims stand, health
  notification emitted; probe failure → `judge:unsupported`, zero child
  sessions created.
...
- Plans written before the redesign (no `wave`/`completedAt`/`judge` fields)
