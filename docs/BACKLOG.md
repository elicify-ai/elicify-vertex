...
- **R-5** residual wrapped-key leaks: 12,532 -> 42 over 180,000 probes. 181 of
  ...
  not the span pass. **Followed up and superseded — see the R-5 entry above
  (2026-08-07).** The characterisation in this line turned out to be wrong in a
  useful way: the single-line class is not "the entropy floor" loosely, it is
  *entirely keys shorter than the 32-char token floor* — at length >= 32,
  55,000 single-line probes leak zero in every alphabet and framing. Two
  zero-over-redaction changes took the wrapped residual to 20/180,000 (uniform)
  and 15/180,000 (adversarial); everything else on the table cost 266-942
  innocent fields to buy it, and lowering the entropy threshold closes *zero*
  wrapped leaks because an ordinary path token measures ~3.89 bits/char, above
  every threshold that would catch low-entropy base32.
