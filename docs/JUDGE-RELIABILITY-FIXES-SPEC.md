# Feature Specification: Judge Reliability & Plan-State Integrity Fixes
...
The **completion judge**, the component the whole redesign was built around, is unreliable in production:
...
- **Verdicts are non-deterministic on identical bytes**: `story:judge-audit` shows S2 in `passed` at 10:26:28 and in `reverted` at 10:26:38 — ten seconds apart, no intervening file change.
- Consequence: three fully-delivered stories are permanently stuck `active`; 82 checkpoint calls for 10 unique tasks; ~44% of post-judge turns were pure loop mechanics; only 8.2% of turns mutated a file.
...
| Symbol | Role | Context |
|--------|------|---------|
| `handleJudgeAudit` (`src/v2/wiring/gate.ts:673`) | modifies | The arbiter stage. Selects unjudged stories (`:679-683`), builds the payload, calls `runJudge`, filters verdicts to the audit set (`:726`), applies them (`:736`). FR-001/FR-004/FR-007 all land here. |
| `StoryEngine.applyJudgeVerdicts` (`src/v2/story.ts`) | modifies | Stamps `story.judge` unconditionally, then reverts failed complete-stories. FR-002 adds re-hydrate-before-mutate. |
...
| `isJudgeVerdictShape` (`src/v2/judge.ts:857-858`) | modifies | Enforces only `pass === true ⟹ all met`. FR-005 adds the converse. |
| `JUDGE_PLAN_FIELD_CHAR_CAP` (`src/v2/judge.ts:157`) | modifies | `= 4000`. FR-006. |
...
| `JUDGE_SYSTEM_PROMPT` (`judge.ts:737`) | modifies | FR-011 — the current text demands observation only before crediting. |
| `scanUnits` / `scanProseField` (`judge.ts`) | modifies | FR-006a — the no-separator pair join is the reproduced root cause. |
...
| `handleJudgeAudit` | **MEDIUM** | `handleSessionIdle`, `tests/v2/gate.test.ts`, `tests/v2/integration-judge.test.ts` | Continuation dispatch, stall accounting |
| `reopenStory` | **MEDIUM** | `elicify_vertex_plan_reopen` tool, gate audit filter | UAT T1 blocked-dependency scenario |
| `isJudgeVerdictShape` | **LOW** | `runJudge` | `tests/v2/judge.test.ts` |
| `JUDGE_PLAN_FIELD_CHAR_CAP` | **LOW** | `buildJudgePayload` | Payload-size tests |
...
| Flow | Relevance |
|---|---|
| `event(session.idle)` → `handleSessionIdle` → `handleJudgeAudit` | Where FR-001, FR-004, FR-007 execute |
| `elicify_vertex_plan_checkpoint` → `StoryEngine.checkpoint` → `persistPlan` | FR-002, FR-003 |
| `runJudge` → `probeCapabilityBounded` → `runSubturn` | FR-005, FR-006, FR-008 |
...
An engineer needs the harness to refuse one specific, empirically-observed class of fabrication: the judge asserting that a file or directory does **not** exist when it demonstrably does (`"renewable-energy.md is MISSING on disk"`, `"ls shows no research/ directory"`).
...
**Independent Test**: With `research/x.md` present, feed the harness a judge item `met:false` whose note says that file is missing; assert the item is dropped, `judge:contradicted` is logged, and — crucially — a sibling item claiming "no sources cited" is NOT dropped.

**Acceptance Scenarios**:

1. **Given** a judge item `met:false` whose note asserts the non-existence of a path, **And** the harness independently confirms that path exists, **Then** that ITEM MUST be dropped from the applied verdict and `judge:contradicted` MUST be logged with the story id, the item id and the path.
...
4. **Given** a judge item asserts a *content* property ("no sources cited", "chart data is hardcoded"), **When** verdicts are applied, **Then** that item MUST NEVER be dropped — content claims are out of scope for this veto.
...
Two plugin runtimes drove one session and both wrote `plan.json`; a judge stamp provably written at 10:26:28 is absent from the final file. An operator must be able to trust that a recorded audit result is not silently destroyed by a peer instance.
...
4. **Given** an idempotent no-op, **When** it returns, **Then** the story's `completedAt` and judge stamp MUST NOT be altered (no laundering of a failed audit via a repeat claim).

---

### User Story 4 — A reopened story can be audited again (Priority: P1)

`reopenStory` clears `story.completedAt`, and the audit filter requires `completedAt !== undefined` — so a story reopened while `complete` becomes permanently invisible to the judge. Observed 3× in the session (`previousStatus: complete → newStatus: complete`).
...
2. **Given** a reopened story with a prior judge stamp, **When** it is re-completed, **Then** the audit MUST re-run (the stale stamp MUST NOT suppress it).
...
**Independent Test**: Feed `runJudge` a verdict with `pass:false` and every item `met:true`; assert `judge:malformed`.
...
3. **Given** a verdict with `pass:false` and an empty `items` array, **When** shape validation runs, **Then** it MUST be accepted (no per-item claim was made; the judge may fail a story wholesale).

---

### User Story 6 — The judge sees the whole plan contract (Priority: P2)

`judge:field-truncated {plan, originalLength:6425, cap:4000}` fired on all 9 runs, plus `judge:field-partial-drop {plan, kept:51, dropped:4}`. The judge then FAILed stories explicitly citing the missing content ("the verifier command is incomplete in the digest", "S5 has no independent verifier set in the digest").
...
1. **Given** a plan digest of 6425 chars, **When** the payload is built, **Then** the full digest MUST be transmitted and no `judge:field-truncated` event MUST fire for `plan`.
...
9 audit cycles, 82 checkpoints for 10 tasks, ~44% of post-judge turns pure loop mechanics, 8.2% mutating a file, no escalation, session ended mid-cycle-10. `maxNoProgressTurns` exists but never fired.
...
1. **Given** a story reverted by the judge `maxStoryReaudits` times, **When** the next audit would revert it again, **Then** the harness MUST NOT revert; it MUST log `judge:reaudit-capped` and emit an operator-visible health notification naming the story.
...
**The original hypothesis is DISPROVEN — see Findings.** `config.ts` registers `maxSteps: 12` (judge) / `1` (intake) and the live host resolves `None` for both, but a live probe proved the judge nonetheless runs **three steps and makes real tool calls**. The field is inert, not harmful. What remains is a correctness-of-comment issue: the code asserts a step budget it does not have.
...
**Independent Test**: `opencode debug agent vertex-judge` shows no `maxSteps`; the registration and its comment no longer claim one.
...
### User Story 9 — The judge is compelled to look before it claims (Priority: P0)

A live probe isolated the true cause of the fabrications. Given only the plan digest (bare relative paths, no instruction forcing an observation), the judge answered from the payload and invented `"ls shows no research/ directory"` for a directory that existed. Given a prompt that named what to check, the same agent called `bash` and `read` and returned a correct verdict. **Identical bytes, opposite verdicts, differing only in prompt specificity.**

**Why this priority**: This is the actual root cause of User Story 1's symptom. FR-001 is the safety net; this is the fix.

**Independent Test**: Re-run the P1 probe (full `runJudge`, payload only) against the fixture worktree; assert the verdict is correct AND the persisted child-session parts contain ≥1 tool call.
...
2. **Given** the judge system prompt, **When** it is issued, **Then** it MUST require at least one filesystem observation before any `met:false` claim about a file's existence or contents.
3. **Given** the P1 fixture (a real file the payload does not quote), **When** the full `runJudge` path runs, **Then** the verdict MUST be correct and the child session MUST show ≥1 tool call in its persisted parts.
...
### User Story 12 — A verdict the judge never looked to make is not applied (Priority: P1)
...
**Independent Test**: force a judge verdict produced with zero tool calls; assert `judge:unverified` and no revert.

**Acceptance Scenarios**:

1. **Given** a judge child session that made **zero** tool calls, **When** its verdict contains any `met:false`, **Then** the verdict MUST NOT be applied, `judge:unverified` MUST be logged, and no revert MUST occur.
2. **Given** a judge child session that made ≥1 tool call, **When** its verdict is applied, **Then** behavior is unchanged.
...
| E4 | `workspaceRoot` unresolvable | No path check is attempted; judge verdict applies, logged inconclusive. |
| E5 | Judge returns verdicts for stories not in the audit set | Existing `judge:off-target` filter still applies, unchanged |
...
| E8 | Idempotent re-checkpoint on a task in a story whose judge FAILED | No-op MUST NOT clear the failing stamp (prevents laundering) |

---

## Behavioral Contract

- When a judge item claims a path does not exist and that path does exist inside the worktree, the system drops that item and reports a contradiction.
...
- When a path claim cannot be checked (unresolvable, escapes the worktree), the system applies the judge verdict and logs the inconclusive check.
...
- The system must **not** execute any string that originates from `plan.json` — `verifiers` included. They are LLM-authored, unvalidated (`isStringArray` only), and stored in an ordinary file; executing them unattended would hand code execution to anyone who can write that file or prompt-inject the agent, bypassing the sandbox the judge subagent itself runs under. *(Round 1, C3.)*
- The system must **not** re-derive a `pass` from a story-level "all verifiers passed" signal, because a story's verifiers can pass while its acceptance items are genuinely unmet — S1's `test -f *.md` passes on a sourceless stub, and "missing `## Sources`" was a CORRECT judge FAIL. A pass may only be re-derived when **every** failing item was individually disproven. *(Round 1, C2.)*
- The system must **not** drop a judge item that makes a content claim ("no sources cited", "data is hardcoded"), because a path-existence check cannot speak to content.
- The system must **not** silently drop a judge verdict it cannot cross-check — every skip must be logged, because an invisible skip is indistinguishable from a pass.
- The system must **not** let an idempotent re-checkpoint clear a failing judge stamp, because that would let the agent launder a failed audit by re-claiming.
- The system must **not** block, retry, or throw into the host on any cross-check failure — fail-open is absolute.
- The system must **not** raise the plan cap so far that redaction cost becomes unbounded; the existing `JUDGE_PAYLOAD_RAW_FIELD_SAFETY_CAP` still applies.

## Integration Boundaries

| System | Data in / out | Contract | Failure behavior | Dev approach |
|---|---|---|---|---|
| Filesystem (path-existence check) | Candidate path + `workspaceRoot` → boolean | `fs.existsSync` on a root-confined resolved path; **no process is ever spawned** | Any throw / unresolvable path → inconclusive → judge verdict applies, logged | Real temp dirs in unit tests |
| opencode host (`session.prompt`, agent resolution) | Judge subturn request → verdict text | Existing `runSubturn` contract | Existing fail-open union (`unsupported`/`unavailable`/`malformed`) | Stub client in tests; one live probe for FR-008 |
...
- **When** the judge returns `{storyId:"S1", pass:false, items:[{itemId:"A1", met:false, note:"research/x.md is MISSING on disk"}]}`
- **Then** item A1 MUST be dropped from the applied verdict
- **And** `judge:contradicted` MUST be logged naming S1, A1 and `research/x.md`
...
**Category**: Edge Case
- **Given** the judge returns `met:false` with note `"chart data is hardcoded, no import"` for a file that exists
- **When** verdicts are applied
...
**Category**: Edge Case
- **Given** story S1 is `complete` with a judge stamp `pass:false`
...
- **When** `buildJudgePayload` runs
- **Then** `payload.plan` MUST equal the input
- **And** no `judge:field-truncated` event for `plan` MUST fire
...
- **When** `buildJudgePayload` runs
- **Then** no `judge:field-partial-drop` event MUST fire for `plan`
...
**Category**: Error Path
- **Given** the judge child session shows zero tool calls in its persisted parts
- **When** its verdict contains a `met:false` item
- **Then** the verdict MUST NOT be applied and `judge:unverified` MUST be logged

#### Scenario: A veto-derived pass is not reported as judge-verified
...
- **Then** it MUST NOT claim the judge "independently verified" that story
...
**Category**: Error Path
- **Given** story S1 has been judge-reverted `maxStoryReaudits` times
- **When** a further audit returns `pass:false` for S1
- **Then** S1 MUST NOT be reverted
- **And** `judge:reaudit-capped` MUST be logged
...
- **When** the full `runJudge` path audits the story
...
| 5 | `pathClaim: corpus of 49 real notes matches expected labels` | Unit | (fixture) | tests/fixtures/judge-replay/item-notes.json |
| 6 | `crossCheck: drops contradicted met:false items` | Unit | US1 AS4 | Pure verdict-filter function |
| 7 | `gate: passing verifiers suppress revert + log judge:contradicted` | Integration | Passing verifiers veto | Full `handleJudgeAudit` |
...
| 18 | `judge: pass:false + all met is malformed` | Unit | pass:false all met | Shape check |
| 19 | `judge: pass:false + empty items accepted` | Unit | Empty items accepted | Shape check |
| 20 | `judge: 6425-char plan transmits whole` | Unit | Plan digest transmits whole | Cap raise |
| 21 | `judge: secret in plan still redacted` | Unit | Secret still redacted | Redaction preserved |
...
| 28 | `judge prompt: demands a filesystem observation before met:false` | Unit | Payload-only audit still observes the filesystem | Prompt-content assertion + live P1 re-run (manual) |
| 21a | `judge: plan verifier lines survive the scan` | Unit | Plan-contract lines are not silently scan-dropped | No `judge:field-partial-drop` for `plan` |
...
| 32 | `gate: veto-derived pass is not reported as judge-verified` | Integration | A veto-derived pass is not reported as judge-verified | FR-001b |
...
| 37 | `gate: zero-tool-call verdict is judge:unverified` | Integration | A verdict with zero tool calls is not applied | FR-014 |
...
| 6 | `complete` (story judge FAIL) | complete | Edge | No-op; stamp untouched | No-op does not launder |
...
3. **Fail-open judge** — `unsupported`/`unavailable`/`malformed` never throw into the host.
...
- **FR-001** *(P2, re-ranked round 2)*: Before applying verdicts, the harness MUST drop any judge item whose `met:false` note asserts the non-existence of a path that exists inside the session `workspaceRoot`, verified with `fs.existsSync` and **no command execution**, using the clause-anchored discriminator specified in User Story 1; if every `met:false` item of a story is dropped, `pass` MUST be re-derived as `true`. Content-property claims MUST never be dropped.
- **FR-001a**: The harness MUST NOT execute any string originating from `plan.json` (`verifiers` or otherwise).
- **FR-001b** *(added round 2, M-15)*: A veto-derived pass MUST remain distinguishable from a genuine judge pass — the stamp MUST retain the original items plus a `contradictedItemIds` marker, and the plan close-out MUST NOT claim the judge "independently verified" a story whose pass was harness-derived.
- **FR-002**: `StoryEngine` MUST route every plan mutation through a private `mutate(sessionID, fn, mode)` seam that acquires the state lock **once**, re-reads `plan.json`, reconciles it with the cached plan **in place** (preserving object identity — `gate.ts:742-753` depends on it), applies `fn`, writes, and releases. `persistPlan` becomes a lock-free inner call used only by `mutate`.
  ...
  - **FR-002c** *(round 2, M-5)*: `acquireStateLock` throws rather than waits. `mutate` MUST retry with bounded jittered backoff (5 × 100 ms) and, on exhaustion, log and abort the write — never throw into the host (FR-010).
...
- **FR-005** *(P1, re-ranked round 2; behavior fixed per M-9)*: A story verdict where `pass === false` and every listed item has `met === true` MUST be **dropped per story** in `handleJudgeAudit`'s verdict filter (`gate.ts:726`) — NOT in `isJudgeVerdictShape`, which is a whole-payload boolean — leaving the story untouched, logging `judge:verdict-contradictory`, and counting toward FR-007's cap. Sibling story verdicts MUST still apply. Repair (manufacturing a pass) is explicitly rejected.
- **FR-006**: The plan digest MUST reach the judge complete. Cap set from the **measured raw length (7518 chars for the audited 6-story plan)** — ≥16000 for headroom. Truncation above the cap MUST still occur and be logged. Audited stories MUST render first.
...
- **FR-007**: Cap consecutive judge reverts per story; at the cap, escalate to the operator instead of reverting. Counter lives in `V2SessionState`, resets on a real user message. Contradictory verdicts (FR-005) count toward it.
...
- **FR-009**: Distinct log events for every skip/contradiction/no-op/merge/cap: `judge:contradicted`, `judge:crosscheck-inconclusive`, `judge:verdict-contradictory`, `judge:unverified`, `checkpoint:idempotent-noop`, `plan:concurrent-merge`, `judge:reaudit-capped`.
- **FR-010**: No change may throw into the host; every new failure path degrades to pre-fix behavior.
- **FR-011** *(P0 — the primary fix)*: The plan digest MUST state the absolute worktree root, and the judge system prompt MUST require a filesystem observation before any `met:false` claim about a file's existence or contents. The current prompt only demands observation before crediting (`judge.ts`: *"Read the files a claim references before crediting it"*) — the negative direction is unconstrained, which is the gap.
...
- **FR-014** *(added round 2, M-14)*: The harness MUST require **≥1 tool call in the judge child session** before applying any `met:false`; otherwise the verdict is `judge:unverified` — logged, not applied. This is the only deterministic floor that covers **content** claims (29 of 49 notes), which FR-001 cannot touch. Readable in-process: the child session's parts via `client.session.messages` before `subturn.ts`'s `finally` deletes it.
...
- **SC-006**: 0 `judge:field-truncated` for `plan` on the 7518-char fixture.
- **SC-006b**: 0 `judge:field-partial-drop` for `plan` on the same fixture (round 2, M-11).
...
- **SC-011** *(FR-014)*: a judge verdict produced with 0 tool calls is logged `judge:unverified` and applies 0 reverts.
...
| FR-001b | US-1 | A veto-derived pass is not reported as judge-verified | 32 |
...
| A6 | FR-008 fallback | **RESOLVED by probe**: judge tool-calls fine; `maxSteps` inert. | Resolved |
...
- Redesigning the judge prompt wholesale or switching judge models.
...
- **H5**: force the judge to return prose → `judge:malformed`, claims stand, no loop.
...
- **H8** *(new)*: a judge run that makes zero tool calls → `judge:unverified`, no revert applied (FR-014).
...
- **C3 — it was an unbudgeted RCE surface.** `verifiers` are LLM-authored (`tools.ts:324`), validated only as `isStringArray` (`story.ts:346`), in an ordinary file. Executing them unattended would bypass the judge's own sandbox. **Resolution: execute nothing (FR-001a).**
...
- **M-9 — FR-005 contradicted its own BDD/US/Symbols.** Now: drop per story in `gate.ts:726`, not `isJudgeVerdictShape`; repair explicitly rejected.
- **M-10 — FR-006a's evidence was wrong and I reproduced the true root cause.** Per-field counts are **11 plan partial-drops / 11 plan truncations** (not 23:13 — that conflated all fields). Reproducing `buildJudgePayload` against the real digest gives `{plan, kept:51, dropped:4}` **exactly**, and the dropped lines are S1/S2/S3's `verifiers:` lines. Isolated the mechanism: `scanUnits` concatenates adjacent units **with no separator**, fusing `…space-exploration.json` + `S2` into the 33-char token `research/space-exploration.jsonS2`, which clears the 3.95 bits/char entropy rule — **neither line trips alone**. Fix: require the offending match to straddle the join.
...
- **M-15 — added FR-001b**: a veto-derived pass must not be reported as "the judge independently verified".
...
- Claimed the judge made "zero tool calls" from the SDK response payload. **Wrong** — the response returns only the final step's parts. The persisted `part` rows show 3 steps and 2 real tool calls. Any future probe must read the DB.
...
**The `maxSteps` hypothesis is DISPROVEN. The judge's tools work.**

| # | Probe | Observed |
|---|---|---|
| P1 | Full `runJudge` (dist), payload only | **Reproduced the production failure in 9.8 s**: `pass:false`, *"research/x.json does not exist… ls shows no research/ directory"* — while the file existed |
